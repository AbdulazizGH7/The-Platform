...
const GroupsPage = () => {
  ...
  const [showRemoveGroupPopup, setShowRemoveGroupPopup] = useState(false); // Separate state for Remove Popup
  ...
  const handleRemoveGroup = (group) => {
    ...
    setShowRemoveGroupPopup(true); // Trigger Remove Group Popup
  };
  ...
  const cancelPopup = () => {
    ...
    setShowRemoveGroupPopup(false); // Close Remove Group Popup
    setShowJoinPopup(false);
    setSelectedGroup(null);
  };
  ...
  return (
    <div className="min-h-screen text-white flex flex-col items-center p-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {groups.map((group, index) => (
            <GroupCard
              key={index}
              title={group}
              onRemove={handleRemoveGroup} // Pass handleRemoveGroup to the GroupCard
      ...
      <Popup
        show={showRemoveGroupPopup} // Display Remove Group Popup
